...
def data_cleaner(dataframe, Datetime_column, time_interval, outcome_column, time_steps_to_evaluate=1, time_steps_look_ahead=0, interprolation_direction='forward'):
    ...
    indexed_df = df_sorted.drop_duplicates(
        Datetime_column, keep='last').set_index(Datetime_column)

    # Creates consistent intervals according to input
    regularized_intervals = indexed_df.asfreq(
        freq=time_interval, method='ffill')

    # separates data that will be the Y from X
    Y_data = regularized_intervals[outcome_column].to_frame(
        name=outcome_column)
    ...
    for i in range(len(X_data) - time_steps_to_evaluate - time_steps_look_ahead):
        ...
        X_reg_intervals.append(
            X_data.iloc[i:(i+time_steps_to_evaluate)].values)
        Y_reg_intervals.append(
            Y_data.iloc[i + time_steps_to_evaluate + time_steps_look_ahead - 1].values)

    # reshapes and converts to np arrays

    X_output_data = np.array(X_reg_intervals).reshape(
        len(X_reg_intervals), X_data.shape[1], time_steps_to_evaluate)
